...
Note: Three productivity checks (delivery count outside min-max range & mean gap > 15min & break > 2 hours between consecutive deliveries) 
are calculated for informational purposes but NOT used as filtering criteria.
...
def filter_problematic_observations(df):
    ...
    # Note: Gap > 2h is now tracked in productivity section, not as a data quality issue
    # We still calculate gap information for reference but don't use it as a filter criterion
    print("  - Note: Breaks > 2h are now tracked in productivity monitoring, not as data quality issues")
...
def identify_issues(row):
    ...
    # Note: Break > 2h is now tracked in the productivity section, not here
            ...
            if pd.notna(date_val) and len(day_group) > 2:  # Only include if more than 2 deliveries
                ...
                for i in range(1, len(enum_sorted)):
                    ...
                    if pd.notna(prev_dt) and pd.notna(curr_dt):
                        ...
                        # Check if this is a long break (> 2 hours = 120 minutes)
                        if gap_minutes > 120:
                            long_breaks.append({
                ...
                if len(gaps) > 0:
                    ...
                    if has_low_count_flag or has_high_count_flag or has_pace_flag or has_long_break_flag:
                        ...
                        flagged_enumerators.append({
                            ...
                            'long_breaks': long_breaks  # List of breaks > 2h
                        })
...
<body>
    <div class="container">
        ...
        <div class="productivity-info">
            <strong>Productivity monitoring:</strong> Enumerators flagged if &lt;{min_deliveries} deliveries OR &gt;{max_deliveries} deliveries OR mean gap between deliveries &gt;{max_mean_gap} minutes OR break &gt; 2 hours
        </div>
        ...
        <div class="productivity-section" id="productivitySection" style="display: none;">
            ...
            <p style="margin-bottom: 15px; color: #856404; font-size: 14px;">Flagged for low productivity (&lt;{min_deliveries} deliveries) or too high productivity (&gt;{max_deliveries} deliveries) or slow pace (mean gap &gt;{max_mean_gap} minutes) or break &gt; 2 hours</p>
            ...
            if (filteredProductivity.length > 0) {{
                ...
                filteredProductivity.forEach(enum_data => {{
                    ...
                    if (enum_data.has_long_break_flag && enum_data.long_breaks) {{
                        ...
                        parts.push(`Break > 2h: ${{breakTexts.join(', ')}}`);
                    }}
