...
4. Distance greater than 50 meters
...
Note: Two productivity checks (delivery count outside min-max range and mean gap > 15min) are 
...
def calculate_productivity_flags(df, min_deliveries=50, max_deliveries=80, max_mean_gap=15):
    """Calculate productivity flags based on fixed thresholds.
    
    Args:
        df: DataFrame with delivery data
        min_deliveries: Minimum expected deliveries (flag if below)
        max_deliveries: Maximum expected deliveries (flag if above)
        max_mean_gap: Maximum mean gap in minutes (flag if above)
    """
    import numpy as np
        ...
        for name, enum_group in day_df.groupby('Full name'):
            ...
            if delivery_count > 2:
                ...
                if len(gaps) > 0:
                    ...
                    # Determine count flag (low or high)
                    if delivery_count < min_deliveries:
                        count_flag = 'Low productivity'
                    elif delivery_count > max_deliveries:
                        count_flag = 'Too high productivity'
                    else:
                        count_flag = ''
...
def filter_field_data(input_file, output_file, min_deliveries=50, max_deliveries=80, max_mean_gap=15):
    ...
    df = calculate_productivity_flags(df, min_deliveries=min_deliveries, max_deliveries=max_deliveries, max_mean_gap=max_mean_gap)
    count_flagged = (df['Delivery_Count_Flag'] != '').sum()
    low_count = (df['Delivery_Count_Flag'] == 'Low productivity').sum()
    high_count = (df['Delivery_Count_Flag'] == 'Too high productivity').sum()
    pace_flagged = (df['Pace_Flag'] != '').sum()
    print(f"   Found {low_count} observations with low productivity (<{min_deliveries} deliveries)")
    print(f"   Found {high_count} observations with too high productivity (>{max_deliveries} deliveries)")
    print(f"   Found {pace_flagged} observations with slow pace (>{max_mean_gap}min mean gap)")
...
def run_filter(input_file, output_file, min_deliveries=50, max_deliveries=80, max_mean_gap=15):
    ...
    # Execute filtering
    total, filtered = filter_field_data(input_file, output_file, min_deliveries, max_deliveries, max_mean_gap)
