...
Calculating the difference in number of buildings per zone
"""
...
ZONES = BASE_DIR / "Delivery_and_Discovery.shp"

# Output files
OUTPUT_CSV = OUTPUT_DIR / "zones_urbanization_2019_2025.csv"
OUTPUT_EXCEL = OUTPUT_DIR / "zones_urbanization_2019_2025.xlsx"

# Final data
FINAL_SHAPEFILE = FINAL_DIR / "zones_urbanization_2019_2025.shp"
FINAL_GEOJSON = FINAL_DIR / "zones_urbanization_2019_2025.geojson"
...
def main():
    ...
    print("\n3. Counting per zone (with centroids)...")
    ...
    buildings_2019_centroids['building_id'] = range(len(buildings_2019_centroids))  # Unique ID
    
    join_2019 = gpd.sjoin(buildings_2019_centroids, zones, how='left', predicate='within')
    
    # Identify and export unassigned buildings
    unassigned_2019 = join_2019[join_2019['index_right'].isna()].copy()
    if len(unassigned_2019) > 0:
        print(f"   ⚠️  {len(unassigned_2019)} buildings 2019 outside zones")
        # Export
        unassigned_2019_export = unassigned_2019.drop(columns=['index_right', 'building_id'], errors='ignore')
        unassigned_2019_export.to_file(OUTPUT_DIR / "unassigned_buildings_2019.geojson", driver='GeoJSON')
        print(f"   ✓ List exported: unassigned_buildings_2019.geojson")
    
    # Count unique buildings per zone (avoid duplicates if overlap)
    counts_2019 = join_2019.groupby('index_right')['building_id'].nunique()
    ...
    buildings_2025_centroids['building_id'] = range(len(buildings_2025_centroids))  # Unique ID
    ...
    if len(unassigned_2025) > 0:
        print(f"   ⚠️  {len(unassigned_2025)} buildings 2025 outside zones")
        # Export
        unassigned_2025_export = unassigned_2025.drop(columns=['index_right', 'building_id'], errors='ignore')
        unassigned_2025_export.to_file(OUTPUT_DIR / "unassigned_buildings_2025.geojson", driver='GeoJSON')
        print(f"   ✓ List exported: unassigned_buildings_2025.geojson")
    
    # Count unique buildings per zone (avoid duplicates if overlap)
    counts_2025 = join_2025.groupby('index_right')['building_id'].nunique()
    ...
    for idx, row in enumerate(top_10.itertuples(), 1):
        name = getattr(row, 'Zone', getattr(row, 'zone', getattr(row, 'Name', getattr(row, 'name', f'Zone {row.Index}'))))
    ...
    with pd.ExcelWriter(OUTPUT_EXCEL, engine='openpyxl') as writer:
        zones_csv.to_excel(writer, sheet_name='All zones', index=False)
