...
# Makeni City Council, Sierra Leone (2025)
# 
...
theme_mcc <- function() {
  theme_minimal(base_size = 12) +
    theme(
      plot.title = element_text(size = 16, face = "bold", color = mcc_colors["primary"]),
...
library(haven)
df <- read_dta("D:/Dropbox/LoGRI/Sierra_Leone/data/3_Final/revenue_reporting/Makeni/MCC_revenue_analysis.dta")
...
p1 <- ggplot(annual_summary, aes(x = factor(year), y = total_revenue / 1e6, fill = type)) +
  ...
  labs(
    ...
    subtitle = "Makeni City Council Property Tax Collection (2025)",
    x = NULL,
    y = "Total Revenue (Millions Le)",
    caption = "Source: MCC Property Tax Database | Note: Data through December 23, 2025"
  ) +
  theme_mcc()
ggsave("D:/Dropbox/LoGRI/Sierra_Leone/output/revenue_reporting/Makeni/01_annual_revenue.png", p1, width = 10, height = 6, dpi = 300)

# 1.1 Weekly Time Series - Revenue by Year
# For Makeni, only 2025 data
years <- 2025
...
for (y in years) {
  ...
  p_weekly <- ggplot(weekly_summary_year, aes(x = year_week, y = total_revenue / 1e6, color = type, fill = type, group = type)) +
    ...
    labs(
      ...
      x = NULL,
      y = "Total Revenue (Millions Le)",
      caption = "Source: MCC Property Tax Database | Dashed line shows smoothed trend"
    ) +
    theme_mcc()
  
  ggsave(paste0("D:/Dropbox/LoGRI/Sierra_Leone/output/revenue_reporting/Makeni/04_weekly_revenue_series_", y, ".png"), 
...
p2 <- ggplot(annual_summary, aes(x = factor(year), y = payment_count / 1000, fill = type)) +
  ...
  labs(
    ...
    caption = "Source: MCC Property Tax Database"
  ) +
  theme_mcc()
ggsave("D:/Dropbox/LoGRI/Sierra_Leone/output/revenue_reporting/Makeni/02_annual_payment_count.png", p2, width = 10, height = 6, dpi = 300)
...
p3 <- ggplot(annual_summary, aes(x = factor(year), y = avg_payment / 1000, color = type, group = type)) +
  ...
  labs(
    ...
    subtitle = "Evolution of mean payment amounts (2025)",
    x = NULL,
  ...
  theme_mcc()
ggsave("D:/Dropbox/LoGRI/Sierra_Leone/output/revenue_reporting/Makeni/03_avg_payment_by_type.png", p3, width = 10, height = 6, dpi = 300)
...
p4 <- ggplot(monthly_summary, aes(x = year_month, y = total_revenue / 1e6, color = type, fill = type, group = type)) +
  ...
  labs(
    ...
    subtitle = "Trend analysis with LOESS smoothing (January - December 2025)",
    x = NULL,
    y = "Total Revenue (Millions Le)",
    caption = "Source: MCC Property Tax Database | Dashed line shows smoothed trend"
  ) +
  theme_mcc()
ggsave("D:/Dropbox/LoGRI/Sierra_Leone/output/revenue_reporting/Makeni/04_monthly_revenue_series.png", p4, width = 14, height = 6, dpi = 300)
...
p5 <- ggplot(monthly_summary, aes(x = year_month, y = payment_count, color = type, group = type)) +
  ...
  theme_mcc()
ggsave("D:/Dropbox/LoGRI/Sierra_Leone/output/revenue_reporting/Makeni/05_monthly_count_series.png", p5, width = 14, height = 6, dpi = 300)


cat("Creating budget vs actual revenue comparison for 2025...\n")
# Budget targets for 2025 - UPDATE THESE VALUES FOR MAKENI
budget_2025 <- data.frame(
  type = c("Property Tax", "Business License"),
  budget = c(40036397.69, 14055715.39)  # NOTE: Update these with actual MCC budget targets
)
...
p_budget <- ggplot(budget_long, aes(x = type, y = amount / 1e6, fill = category)) +
  ...
  labs(
    ...
    caption = "Source: MCC Property Tax Database | Dashed line = Budget target"
  ) +
  theme_mcc() +
  theme(legend.position = "top")
ggsave("D:/Dropbox/LoGRI/Sierra_Leone/output/revenue_reporting/Makeni/budget_vs_actual_2025.png", 
...
ggsave("D:/Dropbox/LoGRI/Sierra_Leone/output/revenue_reporting/Makeni/budget_projection_2025.png", 
...
# 3.1 Seasonality - All months combined (for 2025 only)
...
p7 <- ggplot(seasonal_summary, aes(x = month, y = total_revenue / 1e6, fill = total_revenue)) +
  ...
  scale_fill_gradient(low = mcc_colors["primary"], high = mcc_colors["secondary"]) +
  scale_y_continuous(labels = label_number(suffix = "M", scale = 1),
                     expand = expansion(mult = c(0, 0.15))) +
  facet_wrap(~type, scales = "free_y") +
  labs(
    title = "Revenue Seasonality Pattern",
    subtitle = "Total revenue by month (2025)",
    x = NULL,
    y = "Total Revenue (Millions Le)",
    caption = "Source: MCC Property Tax Database"
  ) +
  theme_mcc()
ggsave("D:/Dropbox/LoGRI/Sierra_Leone/output/revenue_reporting/Makeni/07_seasonality_revenue.png", p7, width = 12, height = 6, dpi = 300)


# 3.2 Heatmap - Year × Month (will show only 2025)
...
p8 <- ggplot(heatmap_data, aes(x = month_num, y = factor(year), fill = total_revenue / 1e6)) +
  ...
  labs(
    ...
    x = NULL,
    y = NULL,
    caption = "Source: MCC Property Tax Database | Note: 2025 data through December 23"
  ) +
  theme_mcc() +
  theme(
...
ggsave("D:/Dropbox/LoGRI/Sierra_Leone/output/revenue_reporting/Makeni/08_heatmap_year_month.png", p8, width = 12, height = 6, dpi = 300)

################################################################################
# PART 6: BANK ANALYSIS
# Makeni City Council Revenue Analysis
################################################################################
...
p16_bar <- bank_summary %>%
  ...
  labs(
    ...
    subtitle = "Total collection per banking partner (2025)",
...
ggsave("D:/Dropbox/LoGRI/Sierra_Leone/output/revenue_reporting/Makeni/16_banks_revenue_combined.png", 
       p16_combined, width = 16, height = 8, dpi = 300)

# 6.2 Bank Monthly Evolution - Payment count excluding Other and MCC
bank_monthly_count <- df_clean %>%
  filter(!bank %in% c("Other", "MCC")) %>%
...
p17 <- ggplot(bank_monthly_count, aes(x = year_month, y = payment_count, color = bank)) +
  ...
  labs(
    ...
    subtitle = "Number of payments over time (2025)",
    x = NULL,
    y = "Number of Payments",
    caption = "Source: MCC Property Tax Database | Excluding Other and MCC"
  ) +
  theme_mcc()
ggsave("D:/Dropbox/LoGRI/Sierra_Leone/output/revenue_reporting/Makeni/17_banks_monthly_payment_count.png", p17, width = 14, height = 7, dpi = 300)

# 6.3 Bank Monthly Evolution - Cumulative revenue excluding Other and MCC
...
p17_cumulative <- ggplot(bank_monthly_cumulative, aes(x = year_month, y = cumulative_revenue / 1e6, color = bank)) +
  ...
  labs(
    ...
    subtitle = "Total accumulated revenue over time (2025)",
    x = NULL,
    y = "Cumulative Revenue (Millions Le)",
    caption = "Source: MCC Property Tax Database | Excluding Other and MCC"
  ) +
  theme_mcc()
ggsave("D:/Dropbox/LoGRI/Sierra_Leone/output/revenue_reporting/Makeni/17_banks_cumulative_revenue.png", 
...
p21 <- df_clean %>%
  ...
  geom_histogram(aes(y = after_stat(density)), bins = 100, 
                 fill = mcc_colors["primary"], alpha = 0.7, color = "white") +
  geom_density(color = mcc_colors["secondary"], linewidth = 1.5) +
...
ggsave("D:/Dropbox/LoGRI/Sierra_Leone/output/revenue_reporting/Makeni/21_payment_distribution.png", p21, width = 12, height = 7, dpi = 300)
...
ggsave("D:/Dropbox/LoGRI/Sierra_Leone/output/revenue_reporting/Makeni/22_payment_categories_time.png", p22, width = 14, height = 7, dpi = 300)

################################################################################
# PART 9: ADVANCED ANALYTICS
################################################################################

# 9.3 Cumulative Revenue by Year (only 2025 for Makeni)
...
ggsave("D:/Dropbox/LoGRI/Sierra_Leone/output/revenue_reporting/Makeni/27_cumulative_revenue.png", p27, width = 14, height = 7, dpi = 300)
...
ggsave("D:/Dropbox/LoGRI/Sierra_Leone/output/revenue_reporting/Makeni/bank_by_payment_quartile.png", 
       p3, width = 12, height = 7, dpi = 300)


# 9.5. Simple linear regression (Bank encoded as factor)
# NOTE: Update the bank factor levels based on actual banks operating in Makeni
# First, check which banks exist in your data:
cat("\nBanks present in Makeni data:\n")
...
stargazer(model, 
          ...
          notes.align = "l",
          out = "D:/Dropbox/LoGRI/Sierra_Leone/output/revenue_reporting/Makeni/regression_table.txt")
...
stargazer(model, 
          ...
          notes.align = "l",
          out = "D:/Dropbox/LoGRI/Sierra_Leone/output/revenue_reporting/Makeni/regression_table.html")

cat("\n=== ANALYSIS COMPLETE ===\n")
cat("All visualizations saved to: D:/Dropbox/LoGRI/Sierra_Leone/output/revenue_reporting/Makeni/\n")
cat("Payment distribution and advanced analytics completed.\n")