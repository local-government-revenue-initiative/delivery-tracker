...
def filter_problematic_observations(df):
    ...
    if 'Delivery Type.1' in df.columns:
        ...
        print("    Skipped: 'Delivery Type.1' column not found in data")
    ...
    for name, name_group in df.groupby('Full name'):
        for date_val, day_group in name_group.groupby('Delivered_Date_temp'):
            if pd.notna(date_val):
                ...
                team = user_name.split('_')[0].split('-')[0] if user_name else ''
...
def identify_issues(row):
    ...
    proof_missing = pd.isna(row.get('Proof Of Delivery Path')) or row.get('Proof Of Delivery Path') == ''
    sign_missing = pd.isna(row.get('Signature Path')) or row.get('Signature Path') == ''
    if proof_missing and sign_missing:
        issues.append("Sign. & Proof missing")
    elif sign_missing:
        issues.append("Signature missing")
    elif proof_missing:
        issues.append("Proof missing")
    
    delivery_type = row.get('Delivery Type')
    delivery_type_1 = row.get('Delivery Type.1')
    if delivery_type_1 is not None:
...
def generate_html_toolbox(df_raw, output_file, city, min_deliveries=50, max_deliveries=80, max_mean_gap=15):
    ...
    for _, row in df_filtered.iterrows():
        ...
        records.append({
            'date': delivered_date.strftime('%Y-%m-%d') if delivered_date else 'No date',
            'team': team,
    ...
    # Get unique dates and teams sorted
    all_dates = set([r['date'] for r in records]) | set([s['date'] for s in delivery_statistics])
    all_teams = set([r['team'] for r in records]) | set([s['team'] for s in delivery_statistics])
    dates = sorted(list(all_dates), reverse=True)
    teams = sorted(list(all_teams))
    
    # Get unique full names for delivery statistics section
...
<body>
    ...
    <script>
        ...
        function applyFilters() {{
            ...
            // Refresh delivery stats with same date/team filters
            applyStatsFilters();
        }}
        ...
        function applyStatsFilters() {{
            ...
            // Update Full Name dropdown based on current date/team
            let available = deliveryStats;
            if (selectedDate) {{
                available = available.filter(s => s.date === selectedDate);
            }}
            if (selectedTeam) {{
                available = available.filter(s => s.team === selectedTeam);
            }}
            const availableNames = [...new Set(available.map(s => s.full_name))].sort();
            const nameSelect = document.getElementById('statsNameSelect');
            const previousName = nameSelect.value;
            nameSelect.innerHTML = '<option value="">-- All enumerators --</option>';
            availableNames.forEach(name => {{
                nameSelect.innerHTML += `<option value="${{name}}">${{name}}</option>`;
            }});
            // Keep previous selection if still available
            if (availableNames.includes(previousName)) {{
            ...
            // Generate table
            const avgDeliveries = filtered.length > 0 ? Math.round(totalDeliveries / filtered.length) : 0;
            
            // Cumulative totals (all dates, respecting team/name filters only)
                        let cumul = deliveryStats;
                        if (selectedTeam) {{
                            cumul = cumul.filter(s => s.team === selectedTeam);
                        }}
                        if (selectedName) {{
                            cumul = cumul.filter(s => s.full_name === selectedName);
                        }}
                        const cumulTotal = cumul.reduce((sum, s) => sum + s.total, 0);
                        const cumulBusiness = cumul.reduce((sum, s) => sum + s.business, 0);
                        const cumulRDN = cumul.reduce((sum, s) => sum + s.rdn, 0);
                        
                        let html = '<table><caption style="caption-side: top; text-align: left; padding-bottom: 8px;">';
                        html += `<div style="margin-bottom: 8px; padding: 10px; background: #fff; border-radius: 4px; border-left: 4px solid #6c757d;">
                            <strong>Total Deliveries: ${{totalDeliveries}}</strong> 
                            (Business: ${{totalBusiness}} | Property: ${{totalRDN}}) | 
                            <strong>Avg Deliveries: ${{avgDeliveries}}</strong> | 
                            <strong>Avg Distance: ${{avgDistanceOverall}}m</strong> | 
                            <strong>Active Enums: ${{filtered.length}}</strong>
                        </div>`;
                        html += `<div style="padding: 8px 10px; background: #e8eaf0; border-radius: 4px; font-size: 12px; color: #495057; border-left: 4px solid #495057;">
                            📈 <strong>Cumulative Total (all dates): ${{cumulTotal.toLocaleString()}}</strong> 
                            (Business: ${{cumulBusiness.toLocaleString()}} | Property: ${{cumulRDN.toLocaleString()}})
                        </div>`;
            html += '</caption><thead><tr>';
            html += '<th>Date</th>';
            html += '<th>Team</th>';
            html += '<th>Full Name</th>';
            html += '<th style="text-align: center;">Business</th>';
            html += '<th style="text-align: center;">Property</th>';
            html += '<th style="text-align: center;">Total</th>';
            html += '<th style="text-align: center;">Cumul</th>';
            html += '<th style="text-align: center;">Avg Distance (m)</th>';
            
            // Pre-calculate cumulative totals per enumerator (all dates, team filter only)
                        const cumulByEnum = {{}};
                        let cumulSource = deliveryStats;
                        if (selectedTeam) {{
                            cumulSource = cumulSource.filter(s => s.team === selectedTeam);
                        }}
                        cumulSource.forEach(s => {{
                            cumulByEnum[s.full_name] = (cumulByEnum[s.full_name] || 0) + s.total;
                        }});
                        
                        filtered.forEach(stat => {{
                            const enumCumul = cumulByEnum[stat.full_name] || 0;
                            html += '<tr>';
                            ...
                            html += `<td style="font-weight: bold; color: #6c3483; text-align: center; background: #f4ecf7;">${{enumCumul.toLocaleString()}}</td>`;
...
def generate_enumerator_pdfs(df_raw, output_dir, city, min_deliveries=50, max_deliveries=80):
    """Generate individual PDF stats for each enumerator.
    ...
    enumerators = sorted(list(set([s['full_name'] for s in delivery_statistics])))
    print(f"\nGenerating individual PDFs for {len(enumerators)} enumerators...")
    
    for enum_name in enumerators:
        # Filter stats for this enumerator
        enum_stats = [s for s in delivery_statistics if s['full_name'] == enum_name]
        enum_stats.sort(key=lambda x: x['date'], reverse=True)
        
        if not enum_stats:
            continue
        
        # Calculate totals
        total_deliveries = sum(s['total'] for s in enum_stats)
        total_business = sum(s['business'] for s in enum_stats)
        total_rdn = sum(s['rdn'] for s in enum_stats)
        avg_deliveries = round(total_deliveries / len(enum_stats)) if enum_stats else 0
        avg_distance = sum(s['avg_distance'] for s in enum_stats) / len(enum_stats) if enum_stats else 0
        team = enum_stats[0]['team'] if enum_stats else ''
        
        # Count data quality issues for this enumerator
        enum_rows = df_raw[df_raw['Full name'] == enum_name]
        issue_counts = {
            'Created On missing': 0,
            'Updated On missing': 0,
            'Delivered On missing': 0,
            'Distance > 50m': 0,
            'Wrong location': 0,
            'Not Delivered': 0,
            'Missing files': 0,
        ...
        for _, row in enum_rows.iterrows():
            if pd.isna(row.get('Created On')) or row.get('Created On') == '':
                issue_counts['Created On missing'] += 1
            if pd.isna(row.get('Updated On')) or row.get('Updated On') == '':
                issue_counts['Updated On missing'] += 1
            if pd.isna(row.get('Delivered On')) or row.get('Delivered On') == '':
                issue_counts['Delivered On missing'] += 1
            distance = pd.to_numeric(row.get('Distance'), errors='coerce')
            if pd.notna(distance) and distance > 50:
                issue_counts['Distance > 50m'] += 1
            if str(row.get('Is Property At Correct Location')).lower() == 'false':
                issue_counts['Wrong location'] += 1
            if ((pd.notna(row.get('Not Delivered Reason')) and row.get('Not Delivered Reason') != '') or
                (str(row.get('Is Property Served')).lower() != 'true')):
                issue_counts['Not Delivered'] += 1
            if ((pd.isna(row.get('Proof Of Delivery Path')) or row.get('Proof Of Delivery Path') == '') or
                ...
                issue_counts['Missing files'] += 1
            delivery_type = row.get('Delivery Type')
            delivery_type_1 = row.get('Delivery Type.1')
            if delivery_type_1 is not None:
                if (delivery_type == 'RDN' and delivery_type_1 != 'Property Rates') or \
                   (delivery_type == 'BUSINESS' and delivery_type_1 != 'License Fees'):
                    issue_counts['Type mismatch'] += 1
        
        # Count productivity flags
        import numpy as np
        productivity_counts = {'Slow pace': 0, 'Break > 1.5h': 0}
        enum_rows_dt = enum_rows.copy()
        enum_rows_dt['_dt'] = enum_rows_dt['Delivered On'].apply(parse_datetime)
        enum_rows_dt['_date'] = enum_rows_dt['_dt'].apply(lambda x: x.date() if pd.notna(x) else None)
        
        for date_val, day_group in enum_rows_dt.groupby('_date'):
            if pd.notna(date_val) and len(day_group) > 2:
                sorted_group = day_group.sort_values('_dt')
                gaps = []
                for i in range(1, len(sorted_group)):
                    prev_dt = sorted_group.iloc[i-1]['_dt']
                    curr_dt = sorted_group.iloc[i]['_dt']
                    if pd.notna(prev_dt) and pd.notna(curr_dt):
                        gap_min = (curr_dt - prev_dt).total_seconds() / 60
                        gaps.append(gap_min)
                        if gap_min > 90:
                            productivity_counts['Break > 1.5h'] += 1
                if gaps and np.mean(gaps) > max_deliveries:  # reuse param
                    pass
                if gaps and np.mean(gaps) > 15:
                    productivity_counts['Slow pace'] += 1
        
        # Build issues table rows
        issues_rows_html = ''
        all_issues = {**issue_counts, **productivity_counts}
        for issue_name, count in all_issues.items():
            if count > 0:
                color = '#e74c3c'
            else:
                color = '#27ae60'
            issues_rows_html += f"""<tr>
                <td>{issue_name}</td>
                <td style="font-weight: bold; color: {color}; text-align: center;">{count}</td>
            </tr>"""
        ...
        # Build table rows
        rows_html = ''
        for stat in enum_stats:
            rows_html += f"""<tr>
                <td style="text-align: center;">{stat['date']}</td>
                <td style="text-align: center;">{stat['team']}</td>
                <td style="text-align: center;">{stat['full_name']}</td>
                <td style="font-weight: bold; color: #27ae60; text-align: center;">{stat['business']}</td>
                <td style="font-weight: bold; color: #e74c3c; text-align: center;">{stat['rdn']}</td>
                <td style="font-weight: bold; color: #2c3e50; text-align: center;">{stat['total']}</td>
                <td style="font-weight: bold; color: #2c3e50; text-align: center;">{stat['avg_distance']:.1f}</td>
            </tr>"""
        
        # Generate HTML for this enumerator
        html_content = f"""<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <style>
        ...
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        body {{ font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; padding: 20px; font-size: 10pt; }}
        h1 {{ color: #2c3e50; margin-bottom: 4px; font-size: 16pt; }}
        h2 {{ color: #495057; margin-bottom: 8px; font-size: 13pt; }}
        .subtitle {{ color: #7f8c8d; margin-bottom: 15px; font-size: 10pt; }}
        .summary {{ padding: 10px; background: #fff; border-radius: 4px; border-left: 4px solid #6c757d; margin-bottom: 8px; font-size: 10pt; }}
        table {{ width: 100%; border-collapse: collapse; font-size: 9pt; }}
        th {{ background: #34495e; color: white; padding: 6px; text-align: center; font-size: 9pt; -webkit-print-color-adjust: exact; print-color-adjust: exact; }}
        td {{ padding: 6px; border-bottom: 1px solid #ddd; }}
        .team-badge {{ display: inline-block; padding: 2px 5px; background: #3498db; color: white; border-radius: 3px; font-weight: bold; font-size: 9pt; -webkit-print-color-adjust: exact; print-color-adjust: exact; }}
    </style>
</head>
<body>
    <h1>{city} - Delivery Statistics</h1>
    <h2>{enum_name}</h2>
    <div class="subtitle">Individual enumerator report</div>
    
    <div class="summary">
        <strong>Total Deliveries: {total_deliveries:,}</strong> 
        (Business: {total_business:,} | Property: {total_rdn:,}) | 
        <strong>Avg Deliveries: {avg_deliveries}</strong> | 
        <strong>Avg Distance: {avg_distance:.1f}m</strong> | 
        <strong>Active Days: {len(enum_stats)}</strong>
    </div>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th style="text-align: center;">Avg Dist. (m)</th>
            </tr>
        </thead>
        <tbody>
            {rows_html}
    ...
    <div style="margin-top: 25px;">
        <h2 style="color: #e74c3c; margin-bottom: 8px; font-size: 13pt;">Data Quality & Productivity Flags</h2>
        <div style="padding: 8px 10px; background: #fff; border-radius: 4px; border-left: 4px solid #e74c3c; margin-bottom: 10px; font-size: 10pt;">
            <strong>Total data quality issues: {total_issues}</strong> out of {total_deliveries} deliveries ({(total_issues/total_deliveries*100):.1f}%)        </div>
        <table>
            <thead>
                <tr>
        ...
        # Generate PDF
        safe_name = enum_name.replace(' ', '_').replace('.', '').replace("'", '')
        ...
        try:
            with open(pdf_path, 'wb') as pdf_file:
                ...
                if pisa_status.err:
                    print(f"  ✗ Error for {enum_name}: PDF generation failed")
                    continue
        except Exception as e:
            print(f"  ✗ Error for {enum_name}: {e}")
            continue
    
    print(f"✓ Generated {len(enumerators)} PDFs in: {output_dir}")
...
def run_toolbox(raw_file, output_file, city, min_deliveries=50, max_deliveries=80, max_mean_gap=15):
    ...
    # Fix date errors in CSV (February dates should be March dates)
    DATE_CORRECTIONS = {
        'February 24': 'March 23',
        'February 25': 'March 24',
        'February 26': 'March 25',
    }
    date_columns = ['Created On', 'Updated On', 'Delivered On']
    for col in date_columns:
        if col in df_raw.columns:
            for wrong, correct in DATE_CORRECTIONS.items():
                mask = df_raw[col].astype(str).str.contains(wrong, na=False)
                if mask.any():
                    df_raw.loc[mask, col] = df_raw.loc[mask, col].str.replace(wrong, correct)
                    print(f"  Fixed {mask.sum()} dates in '{col}': {wrong} → {correct}")
    
    # Create derived columns if missing
    if 'Full name' not in df_raw.columns and 'First Name' in df_raw.columns and 'Last Name' in df_raw.columns:
        df_raw['Full name'] = (df_raw['First Name'].fillna('') + ' ' + df_raw['Last Name'].fillna('')).str.strip()
        print("  Created 'Full name' from 'First Name' + 'Last Name'")
    
    if 'User Name' not in df_raw.columns and 'Username' in df_raw.columns:
        df_raw['User Name'] = df_raw['Username']
        print("  Mapped 'Username' -> 'User Name'")
    
    # Generate HTML toolbox
    generate_html_toolbox(df_raw, output_file, city, min_deliveries, max_deliveries, max_mean_gap)
    
    # Generate individual enumerator PDFs
    stats_enum_dir = os.path.join(os.path.dirname(output_file), 'stats_enum')
    generate_enumerator_pdfs(df_raw, stats_enum_dir, city, min_deliveries, max_deliveries)
